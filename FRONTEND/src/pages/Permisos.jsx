    ...
    <div className="permisos-page">
      <div className="card au-toolbar" style={{ marginBottom: 16 }}>
        <div className="au-toolbar__form">
          ...
          <div className="au-field" style={{ minWidth: 260 }}>
            <label className="au-label">Rol</label>
            <select className="au-input" value={roleId} onChange={onChangeRole} disabled={loadingRoles || roles.length === 0}>
              {roles.map(r => (
      ...
      <div className="card" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        {/* Izquierda: catálogo con búsqueda */}
        <div>
          ...
          />
          {loadingSearch && <p style={{ opacity: 0.8, marginTop: 6 }}>Buscando…</p>}
          <div style={{ marginTop: 8, maxHeight: 400, overflow: "auto", border: "1px solid #333", borderRadius: 8, padding: 8 }}>
            {catalog.map(p => {
              const isAssigned = assignedIds.has(p.id);
              const disabled = mutatingId === p.id;
              return (
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0" }}>
                  <div style={{ fontFamily: "monospace" }}>{p.content_type}.{p.codename}</div>
                  <div style={{ color: "#aaa" }}>— {p.name}</div>
                  <div style={{ marginLeft: "auto" }}>
                    {isAssigned ? (
                      <button type="button" className="au-button au-button--ghost" onClick={() => onRemove(p)} disabled={disabled}>
                        Quitar
                      </button>
                    ) : (
                      <button type="button" className="au-button" onClick={() => onAdd(p)} disabled={disabled || !roleId}>
                        Agregar
                      </button>
                    )}
                  </div>
            ...
            {catalog.length === 0 && !loadingSearch && <p style={{ opacity: 0.7 }}>Sin resultados</p>}
          </div>
        </div>

        {/* Derecha: asignados al rol */}
        <div>
          <label>Asignados {roleObj ? `— ${roleObj.code}` : ""}</label>
          {permError && <p className="error">{permError}</p>}
          {loadingAssigned && <p style={{ opacity: 0.8, marginTop: 6 }}>Cargando…</p>}
          <div style={{ marginTop: 8, maxHeight: 400, overflow: "auto", border: "1px solid #333", borderRadius: 8, padding: 8 }}>
            {assigned.map(p => {
              const disabled = mutatingId === p.id;
              return (
            ...
            {assigned.length === 0 && !loadingAssigned && (
              <p style={{ opacity: 0.7 }}>Este rol no tiene permisos asignados.</p>
            )}
          </div>
        </div>
      </div>
...
function parseErr(e) {
  ...
  try { return JSON.stringify(e); } catch { return "Error de red"; }
}
